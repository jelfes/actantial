...
class TestCompareLabelsScores:
    ...
    def test_no_agreement_accuracy(self):
        ...
        result = compare_labels([df1, df2], metric="accuracy", actant_columns=COLS)
...
class TestCompareLabelsN:
    def test_n_two_coders_no_missing(self):
        df = make_df(
            [1, 2, 3], ["hero", "villain", "hero"], ["sword", "shield", None]
        )
        result = compare_labels([df, df], metric="accuracy", actant_columns=COLS)
        # 1 pair × 3 rows = 3
